...
function Batches() {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/batches").then((result) => setBatches(result.data)).catch((err) => console.log(err));
  }, []);


  const handleDelete = (id) => {
    axios.delete("http://localhost:3001/deleteBatch/" + id).then((res) => {
      console.log(res);
...
const handleSearch = () => {
  const results = batches.filter((batch) => batch.fruittype === searchTerm);
     ...
     <li>
      ...
      <div style={{display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh",fontFamily: 'Poppins, sans-serif',backgroundColor:"#FEF29B"}}>
        <div style={{border: "none", borderRadius: "5px", height:"90vh",width: '90vw',boxShadow: "0 4px 8px rgba(0,0,0,0.3)",backgroundColor:"#ffffff"   }}>
          <Link to="/createbatch " style={{backgroundColor: "black", color: "white", border: "none", padding: "15px", borderRadius: "5px", textDecoration: "none",marginBottom: "10px", display: "inline-block",marginLeft:"10px",marginTop:"20px" }}>Add Batch+</Link>
         ...
         {searchResults.length > 0 ? (
          ...
          <table style={{ width: "100%", textAlign: "center",  borderCollapse: "collapse", marginRight:"80px" }}>
            ...
            <tbody>
              {(searchResults.length > 0 ? searchResults : batches).map((batch) => {
                return (
                  <tr style={{ border: "none" ,height: "50px",fontWeight: "bold"}} key={batch._id}>
                    ...
                    <td>
                      <Link to={`/updatebatch/${batch._id}`} style={{ backgroundColor: "yellow", color: "black", border: "none", padding: "10px 10px", borderRadius: "5px", textDecoration: "none",fontWeight: "bold", }}>Update</Link>
                      <button style={{  marginLeft: "5px", backgroundColor: "red", color: "white", border: "none",  padding: "11px 15px", borderRadius: "5px", textDecoration: "none" ,fontWeight: "bold", }} onClick={() => handleDelete(batch._id)}>Delete</button>
                    </td>
                  </tr>
                );
              })}
